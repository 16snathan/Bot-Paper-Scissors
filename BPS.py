...
def init(data):
    ...
    data.port = '/dev/tty.usbmodem1421' #serial port for arduino communication
    data.baud = 9600 #baudrate of serial data communication
...
def controlHand(cv2,data):
    #send move to bionic hand and return move that was made
    move = ""
...
def playRound(cv2,data): #determine moves and winner, increment their score, increment round
    ...
    if data.difficulty == 'i': data.bionicMove = cheatBionic(cv2,data)
    elif data.difficulty == 'e': data.bionicMove = controlHand(cv2,data)
...
##begin code written by Shiva Nathan and Zane Lee
...
def run():
    ...
    while cap.isOpened():
        ...
        ## Keyboard commands 'b' and 'r' originally by Zane Lee and modified by Shiva Nathan
        ##Keyboard commands 'h' and 'z' exclusively by Shiva Nathan
        ...
        if data.isBgCaptured == 1:  # this part wont run until background captured
            ...
            if length > 0:
                ...
                ##begin gameplay by Shiva Nathan
